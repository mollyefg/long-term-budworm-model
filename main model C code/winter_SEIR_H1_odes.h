...
// --------------------------------Begin ODE system of model L3G1--------------------------------------------//
...
dydt[0] = (-alpha1*phi*y[0]*y[P1sub])/(1+rho1*y[P1sub]) -(alpha2*phi*y[0]*y[P2sub])/(1 + rho2*y[P2sub]) -(alpha3*phi*y[0]*y[P3sub])/(1 + rho3*y[P3sub]) - (gamma1*summation*pow(y[0],gamma4));
dydt[1] = (alpha1*phi*y[0]*y[P1sub])/(1+rho1*y[P1sub]) - m1*delta1*y[1]  - (gamma1*summation*pow(y[1],gamma4));

for(i=2; i <= m1; i++){
	dydt[i] = m1*delta1*y[i-1] - m1*delta1*y[i] - (gamma1*summation*pow(y[i],gamma4));
	if(isnan(dydt[i])== 1 ){
		crash = 1;
		}
	}

dydt[m1+1] = (alpha2*phi*y[0]*y[P2sub])/(1 + rho2*y[P2sub]) - m2*delta2*y[m1+1]  - (gamma1*summation*pow(y[m1+1],gamma4));

for(i=m1 + 2; i <= m1 + m2; i++){
	dydt[i] = m2*delta2*y[i - 1] - m2*delta2*y[i] - (gamma1*summation*pow(y[i],gamma4));
	if(isnan(dydt[i])== 1 ){
			crash = 1;
		}
	}

dydt[m1+m2+1] = (alpha3*phi*y[0]*y[P3sub])/(1 + rho3*y[P3sub]) -m3*delta3*y[m1+m2+1]  - (gamma1*summation*(pow(y[m1+m2+1],gamma4)));
for(i=m1 + m2 + 2; i <= m1 + m2 + m3; i++){
	dydt[i] = m3*delta3*y[i-1] - m3*delta3*y[i]  - (gamma1*summation*pow(y[i],gamma4));
	if(isnan(dydt[i])== 1 ){
			crash = 1;
		}
	}
...
dydt[m1+m2+m3+4] = (gamma1*summation*pow(y[0],gamma4)) +
				   (gamma1*summation*pow(y[1],gamma4)) +

				   (gamma1*summation*pow(y[m1+1],gamma4)) +
				  // (gamma3*Q*summation*pow(y[m1+m2],gamma4))+
				   (gamma1*summation*pow(y[3],gamma4));
				   //+
				   //(gamma3*Q*summation*pow(y[m1+m2+m3],gamma4));
return GSL_SUCCESS;
}
